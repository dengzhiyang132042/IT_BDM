...
public class DaCountAreaAction extends MyBaseAction implements IMyBaseAction{
	...
	int number=0;
	Logger logger=Logger.getLogger(DaCountAreaAction.class);
//----------------------------------------------------	
	...
	private void initCount(Date dateStart,Date dateEnd,List counts) {
		String ghql = "select areaId from DaDemand where DTime >= ? and DTime <= ? group by areaId";
		...
		for (int i = 0; i < glist.size(); i++) {
			if(glist.get(i)!=null){
				...
				DaArea area = (DaArea) ser.get(DaArea.class, (Integer)glist.get(i));
				count.setType(area.getName());
				...
				List list2=ser.find("from DaDemand where areaId = ? and DId in(select DId from DaPerform where PState!='转发' and PTime >=? and PTime <=?)", new Object[]{glist.get(i),count.getsTime(),count.geteTime()});
				if (list2.size()!=0) {//如果为0就不要了
					count.setDaAll(list2.size());
					//获取在该时间范围内故障报修完成的量
					List list3=ser.find("from DaDemand where areaId = ? and DId in(select DId from DaPerform where PState='已完成' and PTime >=? and PTime <=?)", new Object[]{glist.get(i),count.getsTime(),count.geteTime()});
