	...
	private void initCounts(List<WhThreeMeetingCount> counts,String dt) throws ParseException {
		...
		if (d1!=null && d2!=null) {
			if (dt.equals("W")) {
				...
				ca1.set(d1.getTDate().getYear()+1900, d1.getTDate().getMonth()+1, d1.getTDate().getDate());
				ca2.set(d2.getTDate().getYear()+1900, d2.getTDate().getMonth()+1, d2.getTDate().getDate());
				int weekyear = d1.getTDate().getYear()-d2.getTDate().getYear();
