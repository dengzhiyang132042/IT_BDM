...
import com.zs.entity.custom.WhMassageReceiveCount;
...
public class MassageReceiveAction extends MyBaseAction implements IMyBaseAction{
	...
	List<WhMassageReceiveCount> counts;
	
	String filtrate;
	
	String result="massageCount";
	...
	Logger logger=Logger.getLogger(MassageReceiveAction.class);
//----------------------------------------------------	
	...
	public List<WhMassageReceiveCount> getCounts() {
		return counts;
	}
	public void setCounts(List<WhMassageReceiveCount> counts) {
		this.counts = counts;
	}
...
//----------------------------------------------------
	...
	private void initCount(Date dateStart,Date dateEnd,List counts,int num,int orderNumber) {
		List list1=ser.find("select MType from WhMassageReceive where MDate>=? and MDate<=? group by MType",new Object[]{new Timestamp(dateStart.getTime()),new Timestamp(dateEnd.getTime())} );
		if(list1!=null&&list1.size()>0){
			for (int i = 0; i < list1.size(); i++) {
				List list2 = ser.find("from WhMassageReceive where MDate>=? and MDate<=? and MType = ?",new Object[]{new Timestamp(dateStart.getTime()),new Timestamp(dateEnd.getTime()),list1.get(i)});
				if (list2.size()!=0) {//如果为0就不要了
					WhMassageReceiveCount count = new WhMassageReceiveCount();
					//i代表多少种类型
	 ...
	 */
	private void initCounts(List<WhMassageReceiveCount> counts,String dt) throws ParseException {
		//获取两个头尾的时间
		WhMassageReceive d1 = null,d2=null;
		String str="from WhMassageReceive where MDate!=null ";
		String str1="from WhMassageReceive where MDate!=null";
	...
	public String queryOfFenye() throws UnsupportedEncodingException {
		...
		clearSpace();
		counts=new ArrayList<WhMassageReceiveCount>();
		if(id!=null){
			/*
	...
	public String exportExc() throws Exception{
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String filePath=getRequest().getRealPath("/")+"/files/export/whz/网点巴枪维修统计.xls";
		String dayType = "周数";
