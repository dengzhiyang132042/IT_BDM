		...
		try {
			...
			for (int i = 1; i < list.size(); i++) {
				try {
					...
					ZmByNumber byNumber=new ZmByNumber("by"+NameOfDate.getNum(), transToDate(list.get(i)[0]), list.get(i)[1], list.get(i)[2], list.get(i)[3], list.get(i)[4], "", "", list.get(i)[5], list.get(i)[6], transToDate(list.get(i)[0]), transToInt(list.get(i)[9]), null, null, list.get(i)[8],new Timestamp(new Date().getTime()),"有效",type,unum);
					save(byNumber);
				} catch (Exception e) {
