...
public class XtZmNumberServiceImpl extends BaseService implements iXtZmNumberService{
	private Logger log=Logger.getLogger(XtZmNumberServiceImpl.class);
	...
	public void importExcelData(String fileName, File file,String uNum) throws IOException,
		...
		try {
			...
			for (int i = 1; i < list.size(); i++) {
				try {
					XtZmNumber zmNumber=new XtZmNumber("n"+NameOfDate.getNum(),transToDate(list.get(i)[0]),
							list.get(i)[1], list.get(i)[2], list.get(i)[3], list.get(i)[4], list.get(i)[5],
							transToDate(list.get(i)[6]), transToInt(list.get(i)[7]), list.get(i)[8],"注册",
