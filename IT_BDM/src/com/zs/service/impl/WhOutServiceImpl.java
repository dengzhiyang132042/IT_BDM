...
public class WhOutServiceImpl extends BaseService implements iDataImportService{

	private Logger log=Logger.getLogger(WhOutServiceImpl.class);
	...
	public void importExcelData(String fileName, File file,String unum) {
		try {
			...
			for (int i = 1; i < list.size(); i++) {
				try {
					WhOutRepair ot=new WhOutRepair("o"+NameOfDate.getNum(), transToDate(list.get(i)[0]), list.get(i)[1].replace(",", ""), list.get(i)[2],
							list.get(i)[3], list.get(i)[4].replace(",", ""), list.get(i)[5], transToInt(list.get(i)[6]), new Timestamp(new Date().getTime()),
							"注册","有效",unum);
					save(ot);
				} catch (Exception e) {
