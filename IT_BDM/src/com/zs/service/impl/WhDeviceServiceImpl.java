...
public class WhDeviceServiceImpl extends BaseService implements iDataImportService{
	
	private Logger log=Logger.getLogger(WhDeviceServiceImpl.class);
	...
	public void importExcelData(String fileName, File file) {
		try {
			...
			for (int i = 1; i < list.size(); i++) {
				try {
					WhDeviceScout device=new WhDeviceScout("d"+NameOfDate.getNum(), transToDate(list.get(i)[0]), transToTimestamp(list.get(i)[1]), list.get(i)[2], list.get(i)[3], 
