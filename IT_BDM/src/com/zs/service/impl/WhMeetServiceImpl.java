...
public class WhMeetServiceImpl extends BaseService implements iDataImportService{
	...
	public void importExcelData(String fileName, File file,String unum) {
		try {
			...
			for (int i = 1; i < list.size(); i++) {
				try {
					WhMeetingScout m=new WhMeetingScout("m"+NameOfDate.getNum(), transToTimestamp(list.get(i)[0]+" 0:0:0"),
							list.get(i)[1], list.get(i)[2], list.get(i)[3], list.get(i)[4], list.get(i)[5], list.get(i)[6],new Timestamp(new Date().getTime()),"维护","有效",unum);
					save(m);
				} catch (Exception e) {
