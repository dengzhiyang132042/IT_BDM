...
<html>
  <head>
	...
	function queryDetails(dates,datee) {
		...
		a2=datee.split(" ");
		//console.log(a2[0]);
		var path="<%=path%>/zmd!queryOfFenye?cz=no&id=&dates="+a1[0]+"&datee="+a2[0];
		//console.log(path);
	...
	function xiazai(){
		var path ="<%=path %>/zmDataCount!exportExc";
		$.post(
			path,
			function(){
				//console.log(result);
				window.location.href="<%=path%>/files/export/xtz/哲盟数据检查统计.xls";
			}
		);
	}
	</script>
  ...
  <body>
 	<div style="text-align: center;margin-right: 17px;color: white;background-color:#17B4FF;padding: 5px;font-size: 14px;font-weight:bold;">哲盟异常数据统计</div>
 	
 	
	<div style="background-color:white;margin-bottom: 5px;padding: 5px;border: 1px solid #224466;margin-right: 17px;margin-top: 10px;">
    	快速查询
    	<br/>
    	<form action="<%=path %>/zmDataCount!queryOfFenye" method="post">
    		当前查询条件:
